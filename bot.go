...
func (ccb *C0deC0reBot) GetToken() error {
	...
	req_data.Set("client_id", ccb.C0deC0reConfig.ClientID)
	req_data.Set("redirect_uri", ccb.C0deC0reConfig.ListenURL)
	req_data.Set("scope", ccb.C0deC0reConfig.Scope)
	req_data.Set("state", state)

	client := &http.Client{}
	req, _ := http.NewRequest("POST", ccb.C0deC0reConfig.TokenURL, strings.NewReader(req_data.Encode()))
